...
bool DolReader::LoadIntoMemory(bool only_in_mem1) const
{
  ...
  std::ofstream textCode("C:\\users\\darren\\desktop\\SoaCode.txt", std::ios::trunc | std::ios::binary);
  ...
  for (size_t i = 0; i < m_text_sections.size(); ++i)
    ...
    {
      textCode << "Section: " << i << std::endl;
      for (int textIndex = 0; textIndex < m_text_sections[i].size(); textIndex+=2)
      {
        char characterToPrint =
            m_text_sections[i].data()[textIndex] + m_text_sections[i].data()[textIndex + 1];

        //if (characterToPrint >= 6)
        {
          textCode << characterToPrint;
        }
      }
      textCode << std::endl;
  ...
  std::ofstream dataOutput("C:\\users\\darren\\desktop\\SoAData.txt");
  ...
  for (size_t i = 0; i < m_data_sections.size(); ++i)
          ...
          m_dolheader.dataAddress[i] + m_data_sections[i].size() >= Memory::REALRAM_SIZE))
    {
      ...
      dataOutput << "Section: " << i << std::endl;
      for (int textIndex = 0; textIndex < m_data_sections[i].size(); textIndex++)
      {
        char characterToPrint = m_data_sections[i].data()[textIndex];

        // if (characterToPrint >= 6)
        {
          dataOutput << std::hex << characterToPrint;
        }
      }
      dataOutput << std::endl;
